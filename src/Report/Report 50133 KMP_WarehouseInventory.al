report 50135 KMP_WarehouseInventory//T12370-Full Comment
{
    ObsoleteState = Pending;
    ObsoleteReason = 'Report not required(Confirmed by user)';
    Caption = 'Warehouse Inventory';
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/KMP_WarehouseInventory.rdl';
    dataset
    {
        dataitem(ItemLedEntryTemp; "Item Ledger Entry")
        {
            ...
            { }
            column(Company_Name_; CompanyShortNameG."Short Name")
            { }
            ...
            { }
            column(Country_Region_Code; ItemG."Country/Region of Origin Code")
            { }
            column(Job_No_; "Job No.")
            { }
            column("BOE_No_"; CustomBOENumber)
            { }
            column(HS_Code_; ItemG."Tariff No.")
            { }
            ...
            dataitem(ReservationEntry; "Reservation Entry")
            {
                ...
                dataitem(ReservationEntry2; "Reservation Entry")
                {
                    ...
                    { }
                    column(UOM_Reserved_; Abs("Quantity (Base)") / ItemUnitOfMesureG."Qty. per Unit of Measure")
                    { }
                    column(Account_Name_; AccountNameG)
                    { }
            ...
            begin
                ...
                if "Expiration Date" > 0D then begin
                    EVALUATE(ExpiryDaysG, FORMAT(Abs("Expiration Date" - WorkDate)));
                end else
                    ExpiryDaysG := 0;
                ...
                if ItemUnitOfMesureG.FindFirst() then
                    ;
                StorageDaysG := StartDateG - "Posting Date";
    ...
    end;*/ // Avinash: Commented because of logic change in printing the Qty on Hand Base UOM
    local procedure InsertTempRecord(var ItemLedEntryP: Record "Item Ledger Entry")
    var
    ...
    begin
        ...
        repeat
            ...
            if ItemNoG > '' then
                ItemLedEntryG.SetRange("Item No.", ItemNoG);
            if ItemLedEntryG.FindSet() then
                repeat
                    ...
                    If (ItemInventoryPostGrpG = '') then
                        ...
                        if (ItemL."Inventory Posting Group" = ItemInventoryPostGrpG) then
                            InsertTempRecord(ItemLedEntryG)
